...
MapOptimization::MapOptimization(const std::string &name, Channel<FrontEndOut> &input_channel)
  ...
  pubLaserCloudSurround = this->create_publisher<sensor_msgs::msg::PointCloud2>("/global_map", 2);
...
void MapOptimization::saveKeyFramesAndFactor() {
  ...
  // 保存结果
  std::string file = "/home/evan/extra/datasets/my.txt";
  std::fstream output(file.c_str(), std::ios::out | std::ios::app);
  output << previousRobotPosPoint.x << " " << previousRobotPosPoint.y << " " << previousRobotPosPoint.z << std::endl;
  output.close();
...
void MapOptimization::run() {
  ...
  while (rclcpp::ok()) {
    lidarslam::FrontEndOut association;
    _input_channel.receive(association);
    if( !rclcpp::ok() ) break;

    {
      std::lock_guard<std::mutex> lock(mtx);
      // 获得从featureAssociation传来的一帧
      laserCloudCornerLast = association.cornerLast;
      laserCloudSurfLast = association.surfLast;
      // laserCloudOutlierLast = association.cloud_outlier_last;
      
      timeLaserOdometry = association.laserOdometry.header.stamp;

      // 将odometery消息拆解乘6自由度参数放入transformsum中，前三个参数为欧拉角，后三个参数为偏移
      OdometryToTransform(association.laserOdometry, transformSum);

      // 获得新帧到地图的变换参数
      transformAssociateToMap();

      // 提取当前帧附近的关键帧，如果有回环，则提取上一时刻50个关键帧
      // 如果没有回环检测，则提取欧式距离离当前帧最近的50个关键帧
